...
def fetch_training_data_files():
    #file = open("./brats/Task02_Heart/dataset.json").read()
    training_data_files = list()
    path = os.path.join(os.path.dirname(__file__), "Task02_Heart")
    with open("./brats/Task02_Heart/dataset.json") as f:
        data = json.load(f)
        for i in range(len(data["training"])):
            subject_files = list()
            subject_files.append(os.path.join(path,data["training"][i]["image"].replace("./", "")))
            subject_files.append(os.path.join(path,data["training"][i]["label"].replace("./", "")))
